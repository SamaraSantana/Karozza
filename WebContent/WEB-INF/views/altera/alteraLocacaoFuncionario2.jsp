...
<body>
	<form name="form1" action="alteraAluguelFuncionario" method="get">
	<input type="hidden" name="id" id="id" value="${lojaLocacao.id}">

		<div id="nav" style=" width: 900px;margin-top: 30px;">
			<ul class="tabs" style="width: 890px;">
				...
				<li><a href="#LocacaoFinalizar">5 - Finalizar</a></li>
			</ul>
			...
			<div class="tab_container">
				...
				<div class="col-lg-12">
					<div id="LocacaoDataHora" class="cont_tab">
						...
						<h4>
							<div id="lado" class="glyphicon glyphicon-map-marker"></div>Local da
							Reserva:
						</h4>
						...
						<h4>
							<div id="lado" class="glyphicon glyphicon-calendar"></div>Data de Origem:
						</h4>
						...
						<h4>
							<div id="lado" class="glyphicon glyphicon-time"></div>Hora de Origem:
						</h4>
						...
						<h4>
							<div id="lado" class="glyphicon glyphicon-calendar"></div>Data de
							Devolução:
						</h4>
						...
						<h4>
							<div id="lado" class="glyphicon glyphicon-time"></div>Hora de Devolução:
						</h4>
				...
				<div id="LocacaoGrupo" class="cont_tab">
						...
						<c:forEach items="${grupos}" var="grupos">
							<tr>
								...
								<c:if test="${grupos.id == grupoLocacao.id}">
									<td><h4>
											<input type="radio" name="grupo" checked="checked" value="${grupos.id}"
												id="${grupos.nome} - ${grupos.descricao} - R$ ${grupos.preco}" class="radio-custom"
												>
												<label value="${grupos.id}"
												for="${grupos.nome} - ${grupos.descricao} - R$ ${grupos.preco}"
												class="radio-custom-label">Selecionar</label>
										</h4></td>
								</c:if>
					...
					<label class="title" for="title">Veiculos disponíveis:</label>
					<table >
						...
						<c:forEach items="${exemplares}" var="exemplares">
								<tr>
										<td><h4>
										<img class="img"
											src="data:image/jpeg;base64, ${exemplares.foto64 }" />
									</h4></td>

								<td><h4>${exemplares.placa}</h4></td>
							<td><h4>${exemplares.automovel}</h4></td>
							<td><h4>${exemplares.cor}</h4></td>
								<td><h4>${exemplares.status}</h4></td>
									<td><h4>
									<input type="radio" name="exemplar" class="radio-custom"
												value="${exemplares.id}"
												id="${exemplares.id}">
											<label value="${exemplares.id}"
												for="${exemplares.id}"
												class="radio-custom-label">Selecionar</label>
										</h4></td>
								</tr>
						...
						<c:forEach items="${protecoes}" var="protecoes">
							...
							<tr>
								...
								<c:if test="${protecoes.id == protecaoLocacao.id}">
									<td><h4>
											<input type="radio" name="protecao" class="radio-custom" value="${protecoes.id}"
												id="${protecoes.descricao} - R$ ${protecoes.preco}"
												checked="checked">
												<label value="${protecoes.id}"
											for="${protecoes.descricao} - R$ ${protecoes.preco}"
											class="radio-custom-label">Selecionar</label>
										</h4></td>
								</c:if>
									...
									<c:if test="${opcionais.id == opcionalLocacao.id}">
										<td><h4>
												<input type="checkbox" class="checkbox-custom" name="opcional"
													...
													onclick="verificaChecks()" checked="checked">
													<label
											value="${opcionais.id}"
											for="${opcionais.nome} - R$ ${opcionais.preco}"
											class="checkbox-custom-label" onclick="verificaChecks()">Selecionar</label>
											</h4></td>
									</c:if>
								</c:forEach>
							</tr>
 ...
 <div class="col-lg-6">
					...
					<input type="text" name="cpf" id="cpf" class="form-control"  maxlength="11" onKeyPress="return Apenas_Numeros(event);" value="${clientes.cpf}" onBlur="validaCPF(this);">
</div>
...
<div class="col-lg-6">
 ...
 <label for="rg">RG:</label>
        ...
        <input type="text" name="rg" id="rg"  maxlength="12" required="required" class="form-control" onKeyPress="MascaraRG(form1.rg);"  value="${clientes.rg}" onkeypress='return SomenteNumero(event)'>
        </div>
...
<label for="dataNascimento">Data de Nascimento:</label>
        ...
        <input type="date" class="form-control" name="dataNascimento" id="dataNascimento"value="${clientes.dataNascimento}" required="required" maxlength="10" name="date" pattern="[0-9]{2}\/[0-9]{2}\/[0-9]{4}$" min="01-01-2012" max="18-02-2016" />
        </div>
       
 
        
        
        <c:if test="${clientes.sexo == 'Masculino'}">
          <div class="col-lg-6">
         
         <h4>
		<label>
		Sexo:<br/>
		<input type="radio" name="sexo" value="Masculino" id="tipo" checked="checked" required>
		Masculino</label><br/>
		</h4>
		<h4>
		<label>
        ...
        <c:if test="${clientes.sexo == 'Feminino'}">
         <div class="col-lg-6">
		...
		<h4>
		<label>
		<input type="radio" name="sexo" value="Feminino" required checked="checked" id="tipo" required>
		Feminino</label></h4>
        ...
        <input name="cep" type="text" id="cep"  maxlength="9"
               onblur="pesquisacep(this.value);" class="form-control" required="required" value="${clientes.cep}"   onkeypress='return SomenteNumero(event)'  /><br />
         </div>
        ...
        <input type="text" name="rua" class="form-control" required="required" value="${clientes.rua}"  id="rua" >
        </div>
        ...
        <input type="text" name="complemento" class="form-control" value="${clientes.complemento}" id="complemento" >
        </div>
        ...
        <input type="text" name="numRua" class="form-control" required="required"  value="${clientes.numRua}"  onkeypress='return SomenteNumero(event)'  id="numRua" >
        </div>
        ...
        <input name="bairro" type="text" id="bairro" class="form-control" required="required" value="${clientes.bairro}"  />
         </div>
        ...
        <input name="cidade" type="text" id="cidade" class="form-control"  value="${clientes.cidade}" required="required"   /><br />
        ...
        <input name="uf" type="text" id="uf" required="required" class="form-control" value="${clientes.uf}"  />
        ...
        <input type="text" name="cnh"  onkeypress='return SomenteNumero(event)' maxlength="11" class="form-control"value="${clientes.cnh}" required="required"  id="cnh" >
        </div>
        ...
        <input type="date" name="validadeCnh" id="validadeCnh" class="form-control" required="required" value="${clientes.validadeCnh}" maxlength="10" name="date" pattern="[0-9]{2}\/[0-9]{2}\/[0-9]{4}$" min="01-01-2012" max="18-02-2016" />
        </div>
        ...
        <label for="emissaoCnh">Emissão:</label>
          ...
          <input type="date" name="emissaoCnh" id="emissaoCnh" class="form-control" required="required"  value="${clientes.emissaoCnh}" maxlength="10" name="date" pattern="[0-9]{2}\/[0-9]{2}\/[0-9]{4}$" min="01-01-2012" max="18-02-2016" />
        </div>
...
<div id="bts">
				<a href="adm"><input type="button" value="Cancelar" class="bt" /></a>
					...
					jq('input[type="radio"]')
							.click(
									function() {
										...
										if (jq(this).attr("name") == "exemplar") {
											var identificador1 = jq(this)
													.attr("id");
											var ident1 = identificador1;
											document
													.getElementById("divExemplar").innerHTML = "Exemplar: "
													+ ident1;
